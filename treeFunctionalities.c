...
void createIndex(char *binFileName, char *indexFileName) {
    ...
    // VARREDURA NO ARQUIVO DE DADOS E INDEXAÇÃO
    while (1) {
        ...
        if (statusLeitura == 1 && regAtual.removido == '0'){
            ...
            // Chama a função de inserção na Árvore-B passando a chave (codEstacao) 
            // e o ponteiro de dados (byteOffset). O cabeçalho é passado por referência 
            // para alteração do proxRRN e noRaiz durante Splits.
        ...
        // garantindo que o byteOffset permaneça exato.
        rrn++;
    }
...
void searchWithIndex(char *binFileName, char *indexFileName, int nroBuscas) {
    ...
    for (int i = 0; i < nroBuscas; i++){
        ...
        if (criterios.flag_codEstacao == 1){
            // BUSCA INDEXADA
            // Como a chave primária foi requisitada, aciona a Árvore-B.
            int byteOffset = searchKey(arquivoIndice, criterios.regBusca.codEstacao, &headerIndice);

            // A chave existe na árvore
            if (byteOffset != -1) {
                // Posiciona o cursor do disco de dados exatamente no offset físico retornado pelo índice.
                ...
                // para caso o usuário tenha fornecido múltiplos filtros.
...
void insertWithIndex(char *binFileName, char *indexFileName, int nroInsercoes) {
    ...
    for (int i = 0; i < nroInsercoes; i++){
        ...
        if (searchKey(arquivoIndice, novoReg.codEstacao, &headerIndice) != -1){
            continue; // Não insere se a chave já existe
        }

        // REAPROVEITAMENTO A PARTIR DA PILHA DE REMOVIDOS
        long posicaoEscrita;
        int rrnNovoRegistro;

        if (cabecalho.topo == -1){
            // Caso de arquivo sem espaços disponíveis. Insere no final.
        ...
        // Grava o registro de 80 bytes na posição calculada
        fseek(arquivoBinario, posicaoEscrita, SEEK_SET);
        writeRegistros(&novoReg, arquivoBinario, &cabecalho);
        
        // Insere a Chave e o Byte Offset na Árvore-B.
        insertKey(arquivoIndice, posicaoEscrita, novoReg.codEstacao, &headerIndice);
    }

    // VERIFICAÇÃO DE NOME E PAR DE ESTAÇÃO ÚNICOS
    Estacoes contagemFinal = {0}; 
    fseek(arquivoBinario, 17, SEEK_SET);
    Registro regBusca;
    int statusLeitura;
    
    while ((statusLeitura = readRegistros(&regBusca, arquivoBinario)) != 0){
        // Ignora os removidos lógicos
        if (regBusca.removido == '1') continue;

        // Funções de verificação de exclusividade dos campos
        isEstacaoUnica(&contagemFinal, regBusca.nomeEstacao);
        isParUnico(&contagemFinal, regBusca.codEstacao, regBusca.codProxEstacao);
    }

    // Atualiza os valores no cabeçalho
    cabecalho.nroEstacoes = contagemFinal.numEstacoes;
    cabecalho.nroParesEstacao = contagemFinal.numParesEstacao;
...
void deleteWithIndex(char *binFileName, char *indexFileName, int nroRemocoes) {
    ...
    // navegação de ponteiros e atualizações no meio do disco.
    FILE *arquivoBinario = fopen(binFileName, "rb+");
    if (arquivoBinario == NULL) {
        printf("Falha no processamento do arquivo.\n");
        return;
    ...
    for (int i = 0; i < nroRemocoes; i++) {
        ...
        if (criterios.flag_codEstacao == 1) {
            ...
            if (byteOffset != -1) {
                // Move o ponteiro do disco exatamente para o registro alvo
                fseek(arquivoBinario, byteOffset, SEEK_SET);
                readRegistros(&regAtual, arquivoBinario);
                ...
                if (regAtual.removido == '0' && checagemCriteriosBusca(&criterios, &regAtual)) {
                    ...
                    // ATUALIZAÇÃO DO CAMPO DE PARES ÚNICOS
                    if (regAtual.codProxEstacao != -1) {
                        cabecalho.nroParesEstacao--;
                    }

                    // ATUALIZAÇÃO DO CAMPO DE ESTAÇÕES ÚNICAS
            ...
            while (readRegistros(&regAtual, arquivoBinario)) {
                if (regAtual.removido == '0' && checagemCriteriosBusca(&criterios, &regAtual)) {
                    ...
                    // Remove a chave correspondente da Árvore-B
                    removeKey(arquivoIndice, regAtual.codEstacao, &headerIndice);
                    ...
                    // Verificação de remoção de nome de estação única
                    if (regAtual.tamNomeEstacao > 0) {
                        int contagem = 0;
